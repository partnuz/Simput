...
class Smof_Fields_Combobox_Field extends Smof_Fields_Parent_Field{
	...
	function bodyView(){
		...
		?>
			...
			<select <?php $this -> viewName(); ?> <?php if( $this -> options[ 'data_source_format' ] !== false ){?>data-smof-data-source-format="<?php echo $this -> options[ 'data_source_format' ] ;?>"<?php } ?> class="smof-field-combobox <?php echo $this -> formFieldClass(); ?>" >
					...
					case false:
						foreach( $this -> options[ 'options' ] as $field_key => $field_data ){
							...
							if( !is_array( $field_data ) ){
								?>
								<option type="text" <?php selected( $this -> data  , $field_key ); ?> value="<?php echo esc_attr( $field_key ); ?>" ><?php echo htmlspecialchars( $field_data ); ?></option>
								<?php
								...
								?>
								<option type="text" <?php selected( $this -> data  , $field_data[ 'key' ] ); ?> value="<?php echo esc_attr( $field_data[ 'key' ] ); ?>" <?php $this -> addAttributes( $attributes , array( 'typography' ) ); ?> ><?php echo htmlspecialchars( $field_data[ 'val' ] ); ?></option>
								<?php
							}
						...
						if( $this -> options[ 'cache_data_source' ] ){
							...
							foreach( $this -> options[ 'options' ] as $data_source_name => $data_source_data ){
								...
								$this -> args[ 'subframework' ] -> args[ 'framework' ] -> dataSourceAction( 
									$this , 
									$this -> args[ 'subframework' ] -> getFieldId( $this -> args[ 'id' ] ) , 
									array( 
